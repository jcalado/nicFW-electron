...
export const ScanTab = ({ settings }) => (
  ...
  >
    <div className="flex flex-col gap-4">
      ...
      <Input type="number" min={0} max={9} defaultValue="2" value={settings?.squelch} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel info={<></>}>Scan Range</InfoLabel>
      <Input type="number" value={settings?.scanRange} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel
        info={
      ...
      <Input type="number" min={0} max={20} step={0.1} value={settings?.scanPersist} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel
        info={
      ...
      <Input type="number" min={0} max={250} value={settings?.scanResume} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel
        info={
          <>
            ...
            higher value will match less thus speeding it up, but at the cost of potentially missing
            some.
          </>
        }
      >
      ...
      <Input type="number" min={0} max={20} value={settings?.ultraScan} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel
        info={<>How much to refresh the screen when scanning. Larger values equal slower scans.</>}
      >
        Scan Update
      </InfoLabel>
      <Input type="number" min={0} max={50} value={settings?.scanUpdate} />
    </div>
    <div className="flex flex-col gap-4">
    ...
    <div className="flex flex-col gap-4">
      ...
      <Switch checked={!settings?.disableFmt} />
    </div>
    <div className="flex flex-col gap-4">
      <InfoLabel info={<>Default: 47</>}>Squelch Noise Level</InfoLabel>
      <Input type="number" min={45} max={100} value={settings?.sqNoiseLev} />
    </div>
  </div>
)
